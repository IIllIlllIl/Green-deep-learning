...
**运行**:
```bash
sudo python3 mutation_runner.py --experiment-config experiments/all.json
```
...
**运行**:
```bash
sudo python3 mutation_runner.py --experiment-config experiments/default.json
```
...
```bash
# 复制模板
cp experiments/default.json experiments/my_experiment.json

# 修改配置
vim experiments/my_experiment.json

# 运行
python3 mutation_runner.py --experiment-config experiments/my_experiment.json
```
...
1. **experiments/README.md** - 配置文件使用说明
   - 预设配置详解
   - 配置文件格式
   - 使用示例
...
```bash
# 1. 验证配置文件语法
python3 -c "import json; print(json.load(open('experiments/default.json'))['experiment_name'])"

# 2. 快速测试（使用demo配置）
python3 mutation_runner.py --experiment-config experiments/mixed_mode_demo.json
...
# 配置文件模式（新方式）
python3 mutation_runner.py --experiment-config experiments/all.json

# 复现基线（新功能）
python3 mutation_runner.py --experiment-config experiments/default.json
```
