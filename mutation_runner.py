...
Usage:
    ...
    python mutation_runner.py --repo VulBERTa --model mlp \\
                              --mutate all \\
                              --runs 5
"""
...
class MutationRunner:
    ...
    def generate_mutations(self,
        ...
        """Generate mutated hyperparameter sets

        Args:
            repo: Repository name
            model: Model name
            mutate_params: List of hyperparameters to mutate
            num_mutations: Number of mutation sets to generate

        Returns:
            List of mutated hyperparameter dictionaries
        """
        ...
        print(f"📊 Generating {num_mutations} mutation(s) for parameters: {params_to_mutate}")
    ...
    def parse_energy_metrics(self, energy_dir: Path) -> Dict[str, Any]:
        ...
        # Parse GPU power
        gpu_file = energy_dir / "gpu_power.csv"
        if gpu_file.exists():
            try:
                import csv
                powers = []
                ...
                if powers:
                    ...
                    metrics["gpu_power_max_watts"] = max(powers)
                    metrics["gpu_power_min_watts"] = min(powers)
                    # Approximate energy (power * time, assuming 1 second intervals)
                    metrics["gpu_energy_total_joules"] = sum(powers)
    ...
    def run_mutation_experiments(self,
        ...
        for i, mutation in enumerate(mutations, 1):
            ...
            if i < len(mutations):
                print(f"\n⏳ Sleeping 60 seconds to prevent energy interference...")
                time.sleep(60)
    ...
    def run_from_experiment_config(self, config_file: str) -> None:
        ...
        for exp_idx, exp in enumerate(experiments, 1):
            ...
            if exp_idx < len(experiments):
                print(f"\n⏳ Sleeping 120 seconds before next configuration...")
                time.sleep(120)
...
Examples:
  ...
  python mutation_runner.py --repo Person_reID_baseline_pytorch --model densenet121 \\
                            --mutate epochs,learning_rate --governor performance

  # List available models
  python mutation_runner.py --list

  # Run from experiment configuration file
  python mutation_runner.py --experiment-config settings/all.json
  python mutation_runner.py --experiment-config settings/default.json
    ...
    # Initialize runner
    try:
        runner = MutationRunner(config_path=args.config)
    except Exception as e:
        print(f"❌ Error: {e}")
        sys.exit(1)
