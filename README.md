...
**数据文件** (`data/` 目录 ⭐⭐⭐):
- **raw_data.csv**: 合并后的原始数据（87列，836行） - **原始数据源** ⭐⭐⭐
  - 包含所有实验的原始数据（211老实验 + 465新实验 + 110多参数变异 + 50其他补充）
  - 100%训练成功，95.1%能耗完整（795/836），100%性能数据完整
  - 最新更新: 2026-01-04 - 新增110个多参数组合变异实验
  - 验证报告: [tools/data_management/validate_raw_data.py](tools/data_management/validate_raw_data.py)
- **data.csv**: 统一性能指标数据（54列，836行） - **主分析文件** ⭐⭐⭐
  ...
  - 生成脚本: [tools/data_management/create_unified_data_csv.py](tools/data_management/create_unified_data_csv.py)
- **recoverable_energy_data.json**: 可恢复能耗数据（253个实验）
- **backups/**: 数据备份目录（5个备份文件）

**历史数据归档** (`archives/` 目录 ⭐):
- **runs/**: 17个历史运行目录（2025年11-12月）
  - 每个目录包含summary.csv和完整实验数据
- **data_snapshots/**: 历史数据快照
  - summary_old.csv - 老实验数据（93列，211行）
  - summary_new.csv - 新实验数据（80列，265行）
  - collector/ - 数据收集器相关文件
  - archived/ - 已归档的历史数据
  - 其他历史快照文件
...
> **完整重组方案**: [docs/RESTRUCTURE_PLAN_20260105.md](docs/RESTRUCTURE_PLAN_20260105.md)

### 核心数据文件 ⭐⭐⭐

| 文件 | 位置 | 说明 |
|------|------|------|
| **raw_data.csv** | `data/raw_data.csv` | 主数据文件（87列，836行，95.1%完整性）|
| **data.csv** | `data/data.csv` | 精简数据文件（54列，836行，统一并行/非并行字段）|
| **recoverable_energy_data.json** | `data/recoverable_energy_data.json` | 可恢复能耗数据（253个实验）|
| **数据备份** | `data/backups/` | 所有历史备份文件 |
| **数据说明** | `data/README.md` | 数据目录详细说明 |

### 数据处理工具 ⭐⭐

| 目录 | 位置 | 说明 |
|------|------|------|
| **数据管理工具** | `tools/data_management/` | 15个活跃脚本（验证、分析、修复）|
| **配置管理工具** | `tools/config_management/` | 4个配置脚本（生成、验证）|
| **历史脚本** | `tools/legacy/` | 24个历史脚本（归档，仅供参考）|
| **工具说明** | `tools/README.md` | 工具目录详细说明 |

#### 常用数据管理工具

| 脚本 | 用途 | 位置 |
|------|------|------|
| `validate_raw_data.py` | 验证数据完整性 | `tools/data_management/` |
| `analyze_experiment_status.py` | 分析实验状况 | `tools/data_management/` |
| `analyze_missing_energy_data.py` | 分析缺失能耗 | `tools/data_management/` |
| `repair_missing_energy_data.py` | 修复能耗数据 | `tools/data_management/` |
| `append_session_to_raw_data.py` | 追加新实验数据 | `tools/data_management/` |
| `create_unified_data_csv.py` | 生成统一data.csv | `tools/data_management/` |

#### 配置管理工具

| 脚本 | 用途 | 位置 |
|------|------|------|
| `generate_mutation_config.py` | 生成变异配置 | `tools/config_management/` |
| `validate_mutation_config.py` | 验证配置文件 | `tools/config_management/` |
| `verify_stage_configs.py` | 验证阶段配置 | `tools/config_management/` |
| `validate_models_config.py` | 验证模型配置 | `tools/config_management/` |

### 历史数据归档 ⭐

| 目录 | 位置 | 说明 |
|------|------|------|
| **历史运行结果** | `archives/runs/` | 17个历史运行目录（2025年11-12月）|
| **历史数据快照** | `archives/data_snapshots/` | summary_old.csv, summary_new.csv等 |
| **归档说明** | `archives/README.md` | 归档目录详细说明 |

### 训练核心代码 ⭐⭐⭐

| 文件/目录 | 位置 | 说明 |
|------|------|------|
| **mutation.py** | 根目录 | 训练入口脚本 |
| **mutation/** | `mutation/` | 训练核心模块 |
| ├─ runner.py | | 实验运行器 |
| ├─ energy.py | | 能耗监控 |
| ├─ hyperparams.py | | 超参数管理 |
| ├─ dedup.py | | 去重机制 |
...
| 目录 | 位置 | 说明 |
|------|------|------|
| **settings/** | `settings/` | 所有实验配置JSON文件 |
| ├─ 基础配置 | | 快速验证、变异实验等 |
| ├─ 分阶段配置 | | stage1-14配置文件 |
| └─ 归档配置 | `settings/archived/` | 历史配置文件 |

**重要配置文件**:
- `11_models_quick_validation_1epoch.json` - 快速验证（15-20分钟）
- `mutation_all_models_3x_dynamic.json` - 完整变异实验
- `stage_final_all_remaining.json` - 最终阶段（所有剩余实验）
- `multi_param_mutation_all_models.json` - 多参数组合变异

### 文档目录 ⭐⭐⭐

| 目录/文件 | 位置 | 说明 |
|------|------|------|
| **CLAUDE.md** | 根目录 | Claude助手快速指南（v5.2.0）⭐⭐⭐ |
| **README.md** | 根目录 | 本文档（项目总览）⭐⭐⭐ |
| **docs/** | `docs/` | 所有项目文档 |
| ├─ CLAUDE_FULL_REFERENCE.md | | Claude助手完整参考 ⭐⭐⭐ |
| ├─ QUICK_REFERENCE.md | | 快速参考手册 |
| ├─ SCRIPTS_QUICKREF.md | | 脚本快速参考 |
| ├─ JSON_CONFIG_WRITING_STANDARDS.md | | JSON配置书写规范 ⭐⭐⭐ |
| ├─ 11_MODELS_FINAL_DEFINITION.md | | 11个模型完整定义 |
| ├─ RESTRUCTURE_PLAN_20260105.md | | 文件结构重组方案 |
| ├─ results_reports/ | | 实验结果报告（40+个报告）|
| ├─ environment/ | | 环境配置文档 |
| └─ archived/ | | 归档文档 |
...
| 目录/文件 | 位置 | 说明 |
|------|------|------|
| **analysis/** | `analysis/` | 独立的因果推断分析模块 |
| ├─ README.md | | 模块总体介绍 ⭐⭐⭐ |
| ├─ docs/INDEX.md | | 文档总索引 ⭐⭐⭐ |
| ├─ docs/QUESTION1_REGRESSION_ANALYSIS_PLAN.md | | 当前任务方案 ⭐⭐⭐ |
| ├─ scripts/ | | 分析脚本（25+个）|
| ├─ utils/ | | 核心模块（DiBS, DML等）|
| ├─ data/ | | 分析专用数据 |
| └─ results/ | | 分析结果输出 |
...
| 目录 | 位置 | 说明 |
|------|------|------|
| **repos/** | `repos/` | 11个训练模型仓库（保持根目录）|
| ├─ examples/ | | 4个示例模型（mnist, mnist_ff, mnist_rnn, siamese）|
| ├─ VulBERTa/ | | 漏洞检测模型（mlp）|
| ├─ Person_reID_baseline_pytorch/ | | 行人重识别模型（densenet121, hrnet18, pcb）|
| ├─ pytorch_resnet_cifar10/ | | ResNet模型（resnet20）|
| ├─ MRT-OAST/ | | 缺陷定位模型（default）|
| └─ bug-localization-by-dnn-and-rvsm/ | | Bug定位模型（default）|

**重要说明**:
- ✅ repos/目录保持在根目录，未被重组移动
- ✅ 包含所有训练模型的源代码
- ✅ 每个仓库都可独立训练和测试
- ✅ 模型配置文件: `mutation/models_config.json`

### 环境配置

| 目录 | 位置 | 说明 |
|------|------|------|
| **environment/** | `environment/` | 环境配置相关文件 |
| └─ experiments/ | | 实验环境配置 |
...
**查找文件**:
- 核心数据: `data/` 目录
- 数据工具: `tools/data_management/` 目录
- 配置文件: `settings/` 目录
- 模型仓库: `repos/` 目录（根目录）
- 文档: `docs/` 目录或 `CLAUDE.md`
- 历史数据: `archives/` 目录

**更多信息**:
- 完整文件结构图: [CLAUDE.md § 项目结构快览](CLAUDE.md#📁-项目结构快览)
- 重组方案文档: [docs/RESTRUCTURE_PLAN_20260105.md](docs/RESTRUCTURE_PLAN_20260105.md)
- 快速指南: [RESTRUCTURE_QUICKSTART.md](RESTRUCTURE_QUICKSTART.md)
...
| [CLAUDE.md](CLAUDE.md) | **Claude助手快速指南**（精简版，新对话快速入门） ⭐⭐⭐ |
| [docs/CLAUDE_FULL_REFERENCE.md](docs/CLAUDE_FULL_REFERENCE.md) | **Claude助手完整参考**（详细文档，所有上下文） ⭐⭐⭐ |
...
**v4.7.13** (2026-01-05) - 📁 **文件结构重组完成** ✅
- ✅ **项目文件结构重组**: 优化目录结构，提升可访问性
  - 数据文件上浮: `results/` → `data/` (核心数据更易访问)
  - 脚本分类整理: `scripts/` → `tools/data_management/` + `tools/config_management/` + `tools/legacy/`
  - 历史数据归档: 17个历史运行 → `archives/runs/`, 历史快照 → `archives/data_snapshots/`
  - 路径更新: 114个文件路径引用已更新
- ✅ **README文件结构索引**: 新增详细的文件结构索引章节
  - 核心数据文件位置和说明
  - 常用工具脚本索引（数据管理15个 + 配置管理4个）
  - 历史数据归档位置
  - 快速导航指引
- ✅ **文档更新**:
  - CLAUDE.md 版本更新至 v5.2.0
  - 创建3个README文档 (data/, tools/, archives/)
  - 重组方案文档: [docs/RESTRUCTURE_PLAN_20260105.md](docs/RESTRUCTURE_PLAN_20260105.md)
  - 快速指南: [RESTRUCTURE_QUICKSTART.md](RESTRUCTURE_QUICKSTART.md)
- 📊 **改进效果**:
  - 访问优化: 核心数据从3层减少到2层
  ...
  - 历史分离: 17个历史运行已归档
  - 文档完善: 文件结构索引方便快速定位
- 💾 **备份完成**: 核心数据已备份到 `~/nightly_backups/critical_backup_20260105/`

**v4.7.12** (2026-01-04) - 🎯 **多参数组合变异实验完成** ✅
- ✅ **新增110个多参数组合变异实验**: 研究超参数交互效应
  - 配置文件: `settings/multi_param_mutation_all_models.json`
  - 实验时间: 2025-12-30 20:42 - 2026-01-02 10:42 (~85小时)
  - 模型覆盖: 11个模型 × 2种模式（非并行+并行）× 5次重复
  - 训练成功率: 100% (110/110)
  - 能耗数据: 100%完整
  - 变异策略: 同时变异4-5个超参数，对比单参数变异效果
- ✅ **数据文件更新**:
  - raw_data.csv: 726行 → 836行 (+110行)
  - data.csv: 726行 → 836行 (+110行)
  - 自动备份: raw_data.csv.backup_20260104_173354, data.csv.backup_20260104_173400
- ✅ **实验覆盖增强**:
  - 总实验数: 726 → 836 (+15.1%)
  - 并行实验占比: 51.8% (433/836)
  - 非并行实验占比: 48.2% (403/836)
- 📚 **文档完善**:
  - 更新日志: [docs/results_reports/DATA_UPDATE_20260104_MULTI_PARAM_MUTATION.md](docs/results_reports/DATA_UPDATE_20260104_MULTI_PARAM_MUTATION.md) ⭐⭐
  - 数据提取: 使用 `scripts/append_session_to_raw_data.py`
  - 数据生成: 使用 `scripts/create_unified_data_csv.py`
