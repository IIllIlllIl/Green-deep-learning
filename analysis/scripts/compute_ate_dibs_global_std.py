#!/usr/bin/env python3
"""
基于DiBS因果图的全局标准化数据ATE计算脚本

基于DiBS学到的因果图重新计算全局标准化数据的ATE
...
输出:
  ...
  - 保存到: results/energy_research/data/global_std_dibs_ate/
...
def load_dibs_causal_graph(dibs_graph_file: str):
    ...
    参数:
        dibs_graph_file: DiBS因果图CSV文件路径
...
def compute_ate_for_group(
    ...
    print(f"   DiBS因果图文件: {dibs_graph_file}")
    ...
    if not os.path.exists(dibs_graph_file):
        print(f"❌ DiBS因果图文件不存在: {dibs_graph_file}")
        return {"success": False, "error": f"DiBS因果图文件不存在: {dibs_graph_file}"}
    ...
    print(f"   ✅ 提取 {len(whitelist_df)} 条边")
...
def main():
    parser = argparse.ArgumentParser(description="基于DiBS因果图计算全局标准化数据的ATE")
    ...
    print(f"  DiBS因果图目录: {args.dibs_dir}")
