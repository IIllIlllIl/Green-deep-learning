...
目的: 在新生成的6组数据上执行DiBS因果发现
数据源: analysis/data/energy_research/6groups_final/
研究问题:
  - 问题1: 超参数对能耗的影响
  - 问题2: 能耗和性能之间的权衡关系
  - 问题3: 中间变量的中介效应
...
创建日期: 2026-01-16
基于: DIBS_PARAMETER_TUNING_SUCCESS_REPORT_20260105.md 最优配置
"""
...
def generate_summary_report(all_results, output_dir):
    ...
    report_file = output_dir / "DIBS_6GROUPS_FINAL_REPORT.md"

    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("# 6分组数据DiBS因果分析报告\n\n")
        f.write(f"**分析日期**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"**数据源**: analysis/data/energy_research/6groups_final/ (818条记录)\n")
        f.write(f"**任务组数**: {len(all_results)}个\n")
        f.write(f"**DiBS配置**: alpha=0.05, beta=0.1 ⭐, particles=20, steps=5000\n")
        f.write(f"**配置来源**: DIBS_PARAMETER_TUNING_SUCCESS_REPORT_20260105.md (最优配置)\n\n")
        ...
        f.write(f"**使用脚本**: run_dibs_6groups_final.py\n")
        f.write(f"**参考文档**: DIBS_PARAMETER_TUNING_SUCCESS_REPORT_20260105.md\n")
...
def main():
    """主函数"""
    print("="*80)
    print("6分组数据DiBS因果分析 (最优配置)")
    print("="*80)
    print(f"开始时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"数据源: analysis/data/energy_research/6groups_final/")
    ...
    print(f"配置来源: DIBS_PARAMETER_TUNING_SUCCESS_REPORT_20260105.md")
    print("="*80)

    # 创建输出目录
    output_dir = Path(__file__).parent.parent / "results" / "energy_research" / "dibs_6groups_final" / datetime.now().strftime('%Y%m%d_%H%M%S')
