...
def load_interaction_ate(group_id):
    """
    加载交互项ATE数据
...
def main():
    ...
    for group_num, group_id in group_mapping.items():
        ...
        # 加载ATE数据
        causal_effects = load_interaction_ate(group_id)
    ...
    output_dir = "results/energy_research/tradeoff_detection/"
    os.makedirs(output_dir, exist_ok=True)

    # 1. 保存完整JSON
    json_path = os.path.join(output_dir, "all_tradeoffs.json")
    ...
    summary_csv = os.path.join(output_dir, "tradeoff_summary.csv")
    ...
    if all_tradeoff_records:
        ...
        detailed_csv = os.path.join(output_dir, "tradeoff_detailed.csv")
