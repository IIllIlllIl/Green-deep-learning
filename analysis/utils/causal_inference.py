...
class CausalInferenceEngine:
    ...
    def estimate_ate(self,
        ...
        if ref_df is not None:
            # 检查ref_df是否为DataFrame
            if not isinstance(ref_df, pd.DataFrame):
                raise ValueError(f"ref_df必须是pd.DataFrame，实际类型: {type(ref_df)}")

            # 检查所有混淆因素是否在ref_df中
            missing_cols = [conf for conf in confounders if conf not in ref_df.columns]
            if missing_cols:
                raise ValueError(
                    f"ref_df中缺少混淆因素列: {missing_cols}\n"
                    f"ref_df列: {list(ref_df.columns)}\n"
                    f"需要的列: {confounders}"
                )

            # 检查treatment是否在ref_df中
            if treatment not in ref_df.columns:
                raise ValueError(
                    f"ref_df中缺少treatment列: {treatment}\n"
                    f"ref_df列: {list(ref_df.columns)}"
                )
        ...
        if T0 is not None:
            if not isinstance(T0, (int, float)):
                raise ValueError(f"T0必须是标量(int/float)，实际类型: {type(T0)}")
        if T1 is not None:
            if not isinstance(T1, (int, float)):
                raise ValueError(f"T1必须是标量(int/float)，实际类型: {type(T1)}")
        ...
        try:
            ...
            # 创建和拟合DML模型
            # 使用RandomForestRegressor与参考论文保持一致
            from sklearn.ensemble import RandomForestRegressor
            dml = LinearDML(
                model_y=RandomForestRegressor(),
                model_t=RandomForestRegressor(),
                random_state=0  # 与参考论文保持一致
            )
    ...
    def build_reference_df(self,
        ...
        if strategy == "non_parallel":
            ...
            ref_df = data[numeric_cols].mean().to_frame().T
            if self.verbose:
                print(f"  构建ref_df: 全局均值 (1行, {len(numeric_cols)}个数值列)")
            ...
            # 保留groupby列，只对数值列计算均值
            numeric_cols = data.select_dtypes(include=[np.number]).columns
            cols_to_agg = list(set(numeric_cols) - set(groupby_cols))
            ref_df = data.groupby(groupby_cols)[cols_to_agg].mean().reset_index()
            if self.verbose:
        ...
        # 验证ref_df不为空
        if len(ref_df) == 0:
            warnings.warn("构建的ref_df为空，请检查数据和策略")
