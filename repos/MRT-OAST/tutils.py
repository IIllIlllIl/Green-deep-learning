...
def test_prf(pred, labels):
    ...
    accuracy = 1.0 * sum(pred_right) / total if total > 0 else 0.0
    p, r, f1 = cal_prf(pred_all, pred_right, gold, formation=False)

    # Ensure metrics are valid floats
    precision_1 = float(p[1]) if len(p) > 1 and p[1] is not None else 0.0
    recall_1 = float(r[1]) if len(r) > 1 and r[1] is not None else 0.0
    f1_1 = float(f1[1]) if len(f1) > 1 and f1[1] is not None else 0.0

    print(f"Accuracy: {sum(pred_right)}/{total} = {accuracy:.4f} | Precision:{precision_1:.4f} | Recall:{recall_1:.4f} | F1 score:{f1_1:.4f}")
    return accuracy, f1_1
...
def test_prf_detail(pred, labels, origin_pred, pair_data, origin_data, args, max_wrong = 100, to_save = False):
    ...
    with open(f'wrong/wronglist.txt', 'w') as wfile:
        wfile.write("\n".join(wrong_names))
    ...
    # Ensure metrics are valid floats (handle potential None or NaN values)
    precision_1 = float(p[1]) if len(p) > 1 and p[1] is not None else 0.0
    recall_1 = float(r[1]) if len(r) > 1 and r[1] is not None else 0.0
    f1_1 = float(f1[1]) if len(f1) > 1 and f1[1] is not None else 0.0
    ...
    if to_save:
        ...
        with open(csv_path, mode) as rfile:
            if mode == 'w':
            ...
            rfile.write(
                f'{precision_1},{recall_1},{f1_1},{accuracy}\n')
