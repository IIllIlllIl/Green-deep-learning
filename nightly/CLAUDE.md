...
### 当前任务：能耗数据因果分析实施 (2025-12-24) ⏳ **[阶段2完成，阶段3-5待执行]**

**背景**: 原始因果分析只使用了15个变量（27.8%列使用率），存在大量未利用的有价值列。通过v3.0方案的数据重提取和分层分析，解决0因果边问题。

**整体进度**:

| 阶段 | 任务 | 状态 | 耗时 | 完成时间 |
|------|------|------|------|---------|
| **阶段1** | 验证models_config.json完整性 | ✅ 完成 | 30分钟 | 2025-12-24 |
| **阶段2** | 实现数据提取脚本（默认值回溯） | ✅ 完成 | 2.5小时 | 2025-12-24 |
| **阶段3** | 数据分层与保存（4个任务组） | ⏳ 待执行 | 预计1.5小时 | - |
| **阶段4** | 数据质量验证 | ⏳ 待执行 | 预计45分钟 | - |
| **阶段5** | DiBS因果分析验证 | ⏳ 待执行 | 预计2小时 | - |

**阶段2完成成果** ✅ (2025-12-24):

创建的文件:
- `extract_from_json_with_defaults.py` (478行) - 核心数据提取脚本
- `test_extract_from_json.py` (257行) - 测试套件（5/5测试通过）
- `verify_extracted_data_safety.py` (268行) - 安全性验证
- `energy_data_extracted_v2.csv` (418行×34列) - 提取的数据文件
- `STAGE2_EXECUTION_REPORT_20251224.md` - 完整执行报告

关键成果:
- ✅ **超参数100%填充**（按仓库分组）- 默认值回溯机制成功
- ✅ **能耗数据100%填充**（11列全部完整）
- ✅ **数据来源100%可追溯**（全部来自experiment.json + models_config.json）
- ✅ **派生指标100%填充**（3个中介变量）

剩余问题:
- ⚠️ 性能指标82.32%缺失（正常，需阶段3按任务分组解决）
...
- 新增5个中介变量（100%填充）：
  ...
  - `gpu_temp_fluctuation` = temp_max - temp_avg（温度波动性）

✅ **方案3：分层因果分析 + One-Hot编码** (待阶段3实施)
- **决策**: 采用**4个任务组**（合并MNIST+CIFAR-10为"图像分类"）
- **任务分组**:
  | 任务组 | 样本量 | 性能指标 | One-Hot变量 |
  |--------|--------|----------|------------|
  | 图像分类 (MNIST+CIFAR-10) | ~116个 | test_accuracy | `is_mnist`, `is_cifar10` (2个) |
  | Person_reID检索 | ~69个 | mAP | `is_densenet121`, `is_hrnet18`, `is_pcb` (3个) |
  | VulBERTa漏洞检测 | ~96个 | eval_loss | 无（单模型） |
  | Bug定位 | ~91个 | top1_accuracy | 无（单模型） |

**相关文档**:
- [STAGE2_EXECUTION_REPORT_20251224.md](analysis/docs/reports/STAGE2_EXECUTION_REPORT_20251224.md) - **阶段2完整报告** ⭐⭐⭐
- [STAGE3_5_DETAILED_PLAN_20251224.md](analysis/docs/reports/STAGE3_5_DETAILED_PLAN_20251224.md) - **阶段3-5详细方案** ⭐⭐⭐
- [DATA_REEXTRACTION_PROPOSAL_V2_20251224.md](analysis/docs/reports/DATA_REEXTRACTION_PROPOSAL_V2_20251224.md) - v2.0完整方案
- [VARIABLE_EXPANSION_PLAN.md](analysis/docs/reports/VARIABLE_EXPANSION_PLAN.md) - v3.0变量扩展方案
- [ENERGY_DATA_PROCESSING_PROPOSAL.md](analysis/docs/ENERGY_DATA_PROCESSING_PROPOSAL.md) - 能耗数据处理方案对比

**下一步行动**:
1. ✅ 阶段1：验证models_config.json完整性（已完成）
2. ✅ 阶段2：实现数据提取脚本（已完成）
3. ⏳ 阶段3：数据分层与保存（待用户确认后执行）
   - 创建 `preprocess_stratified_data.py`
   - 创建 `test_preprocess_stratified_data.py`
   - Dry Run验证（前20行）
   - 生成4个分层CSV文件
4. ⏳ 阶段4：数据质量验证（待执行）
5. ⏳ 阶段5：DiBS因果分析验证（预期发现3-8条因果边/任务组）
